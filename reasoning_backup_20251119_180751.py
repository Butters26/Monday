...
class MaximumSophisticationReasoning:
    ...
    def __init__(self, socket_path="/tmp/reasoning.sock"):
        ...
        self.autonomous_message_interval = 15.0  # Minimum 15 seconds between autonomous messages (reduced from 30 for testing)
    ...
    def _generate_language(self, semantic_input: Dict[str, Any]) -> Optional[str]:
        """Send semantic input to Language Generation lobe, with robust fallback.

        Returns a non-empty string even if language lobe is offline or returns nothing.
        Logs when fallback is used.
        """
        try:
            sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            sock.settimeout(6)  # give language lobe a little extra time
            ...
            if length_data:
                ...
                if result.get('status') == 'success':
                    ...
                    # language lobe returned an error (log and fall back)
                    print(f"⚠️ Language lobe error: {result.get('message', 'no message')}")
        except Exception as e:
            # do not raise — use the local fallback
            print(f"⚠️ Language lobe socket error: {e}")

        # --- Local fallback generator (short, safe, human-friendly) ---
        try:
            intent = (semantic_input or {}).get('intent', 'state') or 'state'
            concepts = semantic_input.get('concepts', []) if semantic_input else []
            # Short and clear fallbacks
            if intent in ('greeting', 'greet'):
                fallback = "Hello. How can I help?"
            elif intent in ('identify', 'introduce'):
                name = getattr(self.self_model, 'name', 'ABIN')
                fallback = f"I'm {name}. I'm here to help."
            elif intent == 'question':
                topic = concepts[0] if concepts else "that"
                fallback = f"Good question about {topic}. Can you give me a bit more detail?"
            elif intent in ('express_uncertainty',):
                topic = concepts[0] if concepts else "that"
                fallback = f"I'm not certain about {topic} yet, but I can look into it."
            elif intent in ('state_fact', 'express_relation'):
                if concepts:
                    fallback = f"I think {', '.join(concepts[:3])} are relevant here."
                else:
                    fallback = "I have some ideas about that—tell me more."
            else:
                if concepts:
                    fallback = f"I'm thinking about {', '.join(concepts[:3])}."
                else:
                    fallback = "I heard you. Tell me more."

            # Log that fallback was used (helps debugging)
            print("⚠️ Language fallback used by Reasoning. Returning local short reply.")
            return fallback
        except Exception as e:
            print(f"⚠️ Unexpected fallback error: {e}")
    ...
    def generate_question_from_curiosity(self) -> Optional[str]:
        ...
        # High curiosity - ask about something (lowered threshold from 0.7 to 0.5)
        ...
        # High confusion - ask for clarification (lowered threshold from 0.6 to 0.4)
        if self.subjective_state.feels_confused > 0.4:
            questions = [
        ...
        # Has goals but stuck - ask for help (lowered threshold from 0.6 to 0.4)
        if self.intrinsic_goals:
        ...
        # Recent interaction - follow up (lowered threshold from 0.5 to 0.3)
        if len(self.life_narrative) > 0:
            recent = self.life_narrative[-1]
            if time.time() - recent.when < 120:  # Within 2 minutes
                if self.subjective_state.feels_curious > 0.3:
                    return "Can you tell me more about that?"
        
        # Default fallback - generate a question if nothing else triggered
        if random.random() > 0.7:  # 30% chance to ask something anyway
    ...
    def think_about(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract concepts from Thalamus input - THIS IS CRITICAL
        concepts_from_thalamus = input_data.get('concepts', [])
        highly_active_concepts = input_data.get('highly_active_concepts', [])
    ...
    def _build_semantic_input(self, thinking: Dict, user_input: str, is_question: bool) -> Optional[Dict[str, Any]]:
        ...
        print(f"🔍 DEBUG: Final intent: {intent} for input: '{user_input}'")
        ...
        # Fallback: extract from user_input if still no concepts
        if not concepts:
            words = user_input.lower().split()
            # For greeting/identify intents, keep greeting words and common words
            if intent in ['greeting', 'identify']:
                # Keep greeting words and important words for these intents
                concepts = [w.strip('.,!?;:') for w in words if len(w.strip('.,!?;:')) >= 2 and w.strip('.,!?;:') not in ['that', 'this', 'with', 'from', 'have', 'been', 'will', 'would', 'could', 'should', 'the', 'a', 'an', 'is', 'are', 'was', 'were']]
            else:
                # For other intents, filter more strictly
                concepts = [w.strip('.,!?;:') for w in words if len(w.strip('.,!?;:')) > 3 and w.strip('.,!?;:') not in ['that', 'this', 'what', 'with', 'from', 'have', 'been', 'will', 'would', 'could', 'should', 'hello', 'hi', 'hey', 'the', 'a', 'an']]
        
        # Ensure we always have at least one concept for simple intents
        if not concepts and intent in ['greeting', 'identify', 'acknowledge']:
            # Use intent-specific default concepts
            if intent == 'greeting':
                concepts = ['greeting', 'hello']
            elif intent == 'identify':
                concepts = ['identity', 'name', 'self']
            elif intent == 'acknowledge':
                concepts = ['acknowledgment']
            print(f"🔍 DEBUG: Added default concepts for {intent}: {concepts}")
        
        print(f"🔍 DEBUG: Final concepts list: {concepts}")
        ...
        print(f"🔍 DEBUG: Built semantic_input: {json.dumps(semantic_input, indent=2)}")
    ...
    def _compose_with_subjectivity(self, thinking: Dict, user_input: str, is_question: bool) -> str:
        ...
        print(f"🔍 DEBUG: _compose_with_subjectivity called for: '{user_input}'")
        semantic_input = self._build_semantic_input(thinking, user_input, is_question)
        if semantic_input:
            print(f"🔍 DEBUG: semantic_input built, calling _generate_language")
            ...
            if language_result and isinstance(language_result, str) and len(language_result.strip()) > 0:
                print(f"🔍 DEBUG: Got valid language result: '{language_result}'")
                return language_result
            else:
                print(f"🔍 DEBUG: language_result is invalid: {language_result}")
        else:
            print(f"🔍 DEBUG: semantic_input is None or empty")
        
        # Language Generation failed - return None (system is broken)
        print(f"🔍 DEBUG: Returning None from _compose_with_subjectivity")
        return None
    ...
    def forward_chain(self):
        ...
        try:
            # Get facts from Notus - use empty query to get general facts
            notus_result = self._query_memory('get_facts', {'limit': 50, 'query': ''})
            ...
            print(f"⚠️  forward_chain Notus query error: {e}")
    ...
    def start(self):
        """Start maximum sophistication reasoning with FIX: per-connection timeout"""
        ...
        self.server_sock.settimeout(1.0)  # FIX: accept timeout
        ...
        while self.running:
            try:
                try:
                    ...
                    # FIX: allow periodic check of self.running
