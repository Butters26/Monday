...
class BrainWorker(QThread):
    ...
    def send_to_thalamus(self, text: str) -> Dict[str, Any]:
        """Send message to Thalamus coordinator with FIX: recv_all"""
        try:
            ...
            length_data = _recv_all(sock, 4, timeout=30)
            ...
            response_data = _recv_all(sock, response_length, timeout=30)
            
            sock.close()
            return json.loads(response_data.decode('utf-8'))
            
        except Exception as e:
            return {'status': 'error', 'message': f'Monday offline: {str(e)}'}
...
class MondayInterface(QMainWindow):
    ...
    def check_autonomous_messages(self):
        """Check for autonomous messages from Monday with FIX: recv_all"""
        try:
            sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect("/tmp/thalamus.sock")
            ...
            # FIX: use recv_all
            length_data = _recv_all(sock, 4, timeout=1)
            response_length = struct.unpack('!I', length_data)[0]
            ...
            response_data = _recv_all(sock, response_length, timeout=1)
