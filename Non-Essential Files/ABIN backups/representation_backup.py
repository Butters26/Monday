...
class RepresentationLayer:
    ...
    def _initialize_basic_concepts(self):
        ...
        # Basic emotions
        emotions = ['happy', 'sad', 'angry', 'excited', 'worried', 'calm', 'curious', 'proud']
        for emotion in emotions:
            self.create_concept(emotion, 'emotion', {'intensity_range': (0.0, 1.0)})
        
        # Basic actions
        actions = ['think', 'feel', 'remember', 'learn', 'speak', 'listen', 'see', 'understand']
        for action in actions:
            self.create_concept(action, 'action', {})
        
        # Basic relations
        self.create_concept('is_a', 'relation', {'description': 'type/category relationship'})
        self.create_concept('has_property', 'relation', {'description': 'property relationship'})
        self.create_concept('causes', 'relation', {'description': 'causal relationship'})
        self.create_concept('similar_to', 'relation', {'description': 'similarity relationship'})
        self.create_concept('opposite_of', 'relation', {'description': 'opposition relationship'})
        
        print(f"✅ Initialized {len(self.concepts)} basic concepts")
