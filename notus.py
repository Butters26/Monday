...
class EnhancedMondayMemorySystem(SuperhumanMemorySystem):
    ...
    def _analyze_user_intent(self, message: str, recent_messages: List[str]) -> Dict[str, Any]:
        """Analyze what the user is trying to do/say"""
        message_lower = message.lower()
        recent_context = '\n'.join(recent_messages).lower() if recent_messages else ""

        # Look for writing-related intent
        if any(word in message_lower for word in ['write', 'writing', 'story', 'book', 'chapter']):
            if 'help' in message_lower or 'stuck' in message_lower:
                return {'type': 'writing_help', 'focus': 'problem_solving'}
            elif any(word in message_lower for word in ['idea', 'plot', 'character']):
                return {'type': 'writing_help', 'focus': 'brainstorming'}
            else:
                return {'type': 'writing_discussion', 'focus': 'general'}

        # Look for emotional sharing
        if any(word in message_lower for word in ['feel', 'feeling', 'felt', 'tired', 'stressed']):
            return {'type': 'emotional_sharing', 'focus': 'feelings'}

        # Look for questions/information seeking
        if '?' in message or any(word in message_lower for word in ['how', 'what', 'why', 'when', 'where']):
            return {'type': 'question', 'focus': 'information'}

        # Look for social/casual chat
        if any(word in message_lower for word in ['hi', 'hey', 'hello', 'thanks', 'thank']):
            return {'type': 'social', 'focus': 'greeting'}

        # Look for task/action requests
        if any(word in message_lower for word in ['can you', 'could you', 'help me', 'please']):
            return {'type': 'request', 'focus': 'task'}

        # Default to conversation continuation
        return {'type': 'conversation', 'focus': 'general'}
...
class NotusProcess:
    """Notus as an independent process that communicates via Unix sockets"""
    
    def __init__(self, socket_path="/tmp/notus.sock"):
        self.memory_system = EnhancedMondayMemorySystem()
        self.socket_path = socket_path
        self.running = True
        
    def start(self):
        """Start Notus as independent process"""
        ...
        print(f"🧠 Notus Memory Lobe: Online at {self.socket_path}")
    ...
    def process_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if msg_type == 'store':
            ...
            memory_type = message.get('memory_type', 'episodic')
            self.memory_system.store_memory(content, memory_type=memory_type)
            ...
            context = self.memory_system.get_smart_context(user_input)
